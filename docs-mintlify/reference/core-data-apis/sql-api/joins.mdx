...
{/* [DataOS fork] Renamed __cubeJoinField → __joinField throughout this section */}

## `CROSS JOIN` and `__joinField`

The SQL API also supports joins through the `__joinField` virtual column, which
...
Usage of `CROSS JOIN` or `__joinField` instructs Cube to perform join as it's defined
...
```sql
cube=> SELECT p.name, SUM(o.count) FROM orders o LEFT JOIN products p ON o.__joinField = p.__joinField GROUP BY 1 LIMIT 5;
           name           | SUM(o.count)
--------------------------+--------------
...
```sql
cube=> EXPLAIN SELECT p.name, sum(o.count) FROM orders o LEFT JOIN products p ON o.__joinField = p.__joinField GROUP BY 1 LIMIT 5;
   plan_type   |         plan
---------------+-----------------------
...
In most of the BI tools you'd use `__joinField` to define joins between cube
